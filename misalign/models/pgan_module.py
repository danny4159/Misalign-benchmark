...
class PixelGANModule(BaseModule):
    ...
    def backward_G(self, real_a, real_b, fake_a, fake_b, lambda_l1, lambda_vgg, weight=None):
        ...
        if weight is not None:
            loss_G_A = torch.mean(loss_G_A * weight_batch)
        else:
        ...
        if weight is not None:
            loss_G_B = torch.mean(loss_G_B * weight_batch)
        else:
        ...
        if weight is not None:
            loss_L1_A = torch.mean(loss_L1_A * weight_spatial)
        else:
        ...
        if weight is not None:
            VGG_loss_A = torch.mean(VGG_loss_A * weight_spatial)
        else:
        ...
        if weight is not None:
            VGG_loss_B = torch.mean(VGG_loss_B * weight_spatial)
        else:
    ...
    def training_step(self, batch: Any, batch_idx: int):
        ...
        with optimizer_G.toggle_model():
            loss_G = self.backward_G(real_a, real_b, fake_a, fake_b, self.params.lambda_l1, self.params.lambda_vgg, weight=w)
        ...
        with optimizer_D_B.toggle_model():
            ...
            if w is not None:
                weight_batch = torch.mean(w, dim=(1, 2, 3),keepdim=True)
                loss_D_B = torch.mean(loss_D_B * weight_batch)
            else:
    ...
    def configure_optimizers(self):
        ...
        """
        self.optimizer_G = self.hparams.optimizer(params=itertools.chain(self.netG_A.parameters()))
